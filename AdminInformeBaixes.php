    ...
    <div class = "custom-select-container glass-container">
        ...
        <table id="tableinforme" name="tableinforme" class="table table-striped table-hover table-condensed" style="text-align:center; width: 100%; ">
            ...
            foreach ($persones as $empleat)
            {
                ...
                $totaldiesany = 0;
                echo "<tr>";
                echo '<td style="width:1%; white-space:nowrap;">'.$empleat["cognom1"]." ".$empleat["cognom2"].", ".$empleat["nom"].'</td>';
                ...
                for($i=0,$j=1;$i<12;$i++)
                {
                    ...
                    if(($mes+$i)<=12)
                    {
                        $diesespecialsmes=$dto->calculaDiesEspecialsPerIdMesAnyIdExcep($empleat["idempleat"],$pmes,$anya,$tipusexcep);
                        if($diesespecialsmes>0) {$color = 'style="background-color: rgb(255,255,128)"';}
                        echo '<td '.$color.'>'.$diesespecialsmes.'</td>';
                    }
                    else
                    {
                        $diesespecialsmes=$dto->calculaDiesEspecialsPerIdMesAnyIdExcep($empleat["idempleat"],$j,$any,$tipusexcep);
                        $j++;
                        if($diesespecialsmes>0) {$color = 'style="background-color: rgb(255,255,128)"';}
                        echo '<td '.$color.'>'.$diesespecialsmes.'</td>';
                    }
                    $totaldiesany+=$diesespecialsmes;
                }
                if($totaldiesany>=45) echo '<td style="background-color: rgb(255,128,128)">';
                else echo '<td>';
                echo $totaldiesany."</td></tr>";
