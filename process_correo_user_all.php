...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    ...
    $query = "SELECT user, contrasenya, email, nom, cognom1 FROM empleat WHERE enplantilla = 1";
    $result = mysqli_query($conn, $query);

    if ($result && mysqli_num_rows($result) > 0) {
      

        // Configura tu instancia de PHPMailer aquí
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // Aquí puedes hacer algo con $msg si lo necesitas
        }
    } else {
        echo "No se encontró información de empleados en la base de datos.";
    }
