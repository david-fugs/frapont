...
// Cerrar el resultado de la consulta
 ...
 ';
    ...
    public function seleccionaEmpPerEmpresaActius($idsubempresa) {
        $sql = 'select e.idempleat, e.cognom1, e.cognom2, e.nom, e.email, e.enplantilla from empleat as e where e.idsubempresa = "'.$idsubempresa.'" and e.enplantilla = 1 order by e.cognom1, e.cognom2, e.nom';
        return $this->getDb()->executarConsulta($sql);
    }
    ...
    public function seleccionaHoresTeoriques($id) {
        ...
        $sql_holiday = "
            ...
            FROM festiu AS f
                ...
                join situat as s on s.idlocalitzacio = l.idlocalitzacio
                join empleat as e on e.idEmpleat = s.idempleat
            where e.idEmpleat = $id AND YEAR(f.dataany) = $year AND YEAR(s.datainici) = $year AND YEAR(s.datafi) = $year
        ";
        $holiday_days = [];
    ...
    public function monthlyEmployeeCalendar($id, $any, $mes, $percent, $lng) {
        ...
        $year = 2023; // Cambia el año según sea necesario
        if (date('L', strtotime("$year-01-01"))) $daysInYear = 366; // Año bisiesto
        else $daysInYear = 365; // Año no bisiesto
        $daysOfYear = [];
        for ($day = 0; $day <= $daysInYear - 1; $day++) {
    ...
    ";
        ...
        FROM festiu AS f
            ...
            join situat as s on s.idlocalitzacio = l.idlocalitzacio
            join empleat as e on e.idEmpleat = s.idempleat
        where e.idEmpleat = $id AND YEAR(f.dataany) = $any AND YEAR(s.datainici) = $any AND YEAR(s.datafi) = $any
        ";
        $rs_holiday = $this->getDb()->executarConsulta($sql_holiday);
    ...
    public function monthlyEmployeeCalendarMonth($id, $any, $month, $lng) {
        ...
        FROM festiu AS f
            ...
            join situat as s on s.idlocalitzacio = l.idlocalitzacio
            join empleat as e on e.idEmpleat = s.idempleat
        where e.idEmpleat = $id AND YEAR(f.dataany) = $any AND MONTH(f.dataany) = $month AND YEAR(s.datainici) = $any AND YEAR(s.datafi) = $any
        ";
        $rs_holiday = $this->getDb()->executarConsulta($sql_holiday);
        //CAMBIO VALORES DE TITLE, STYLE DE LAS FECHAS FESTIVOS
        if((!empty($rs_holiday))&&($prifestius==1))
