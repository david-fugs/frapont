...
{
    if(($i==0)&&($id!=$rp["idempleat"])) {$idpersant=$rp["idempleat"];}
    if(($i>0)&&($id==$rp["idempleat"])) {$persant='<button class="btn-neutro" name="id" value="'.$idpersant.'" formaction="AdminHorarisEmpleat.php" title="'.$dto->__($lng,"Anterior").'"><span class="glyphicon glyphicon-arrow-left"></span></button>';}
    else{$idpersant=$rp["idempleat"];}
    $persopt.='<option value="'.$rp["idempleat"].'">'.$rp["cognom1"].' '.$rp["cognom2"].' '.$rp["nom"].'</option>';
    if($chkpers==1) {$persseg='<button class="btn-neutro" name="id" value="'.$rp["idempleat"].'" formaction="AdminHorarisEmpleat.php" title="'.$dto->__($lng,"Següent").'">
    ...
    <div class="col-lg-12">
        ...
        <div class="col-lg-3">
            <div style="text-align: center"> <h2 class=""><?php echo $dto->mostraNomEmpPerId($id);?>  </h2><br>
                <label class=""><?php echo $dto->__($lng,"Hores")." ".$dto->__($lng,"Teòriques")." ".$dto->__($lng,"any")." "."2023";?>:</label> <strong> <?php echo $dto->seleccionaHoresTeoriques($id); ?> </strong><br>
